...
class DocumentedFunction(SelfDocumenting):
    def __init__(self, func, unit, description=None, docfunc=None, docargs=[]):
        self.func = func
        self.unit = unit
    ...
    def format(self, lang):
        ...
        for arg in self.docargs:
            if isinstance(arg, DocumentedFunction):
                ...
                elements[arg] = formatting.ParameterFormatElement(arg, formatting.get_parametername(arg, lang), 'todo')
                argelts.append(formatting.get_parametername(arg, lang))
                alldeps.append(arg)
            else:
                argelts.append(formatting.FormatElement(str(arg), 'todo', is_primitive=True))
        ...
        if self.description in formatting.functions_known:
            ...
            if lang == 'latex':
                elements.update({'main': formatting.FormatElement("%s(%s)" % (knownelt.repstr, ', '.join(map(formatting.get_repstr, argelts))), 'todo', [knownelt.extname] + alldeps)})
            else:
                elements.update({'main': formatting.FormatElement("%s(%s)" % (knownelt.repstr, ', '.join(map(formatting.get_repstr, argelts))), 'todo', [knownelt.extname] + alldeps)})
        ...
        if lang == 'latex':
            elements.update(latextools.call(self.docfunc, self.unit, self.description, *argelts))
        elif lang == 'julia':
            elements.update(juliatools.call(self.docfunc, self.unit, self.description, *argelts))
        else:
