...
def test_Instabase():
    ...
    app1 = calc.test()
    app2 = calc.test()
    for yearresult in app1.push(1000 + np.arange(365), [-10] * 300 + [10] * 65):
        np.testing.assert_equal(True, False) # Should get nothing here
    for yearresult in app2.push(1000 + np.arange(365), [10] * 365):
        np.testing.assert_equal(True, False) # Should get nothing here
    for yearresult in app1.push(2000 + np.arange(365), [-10] * 65 + [10] * 300):
        if yearresult[0] == 1:
            np.testing.assert_equal(yearresult[1], 65. / 300.)
        if yearresult[0] == 2:
            np.testing.assert_equal(yearresult[1], 1.)
    for yearresult in app2.push(2000 + np.arange(365), [10] * 365):
        if yearresult[0] == 1:
            np.testing.assert_equal(yearresult[1], 1.)
        if yearresult[0] == 2:
            np.testing.assert_equal(yearresult[1], 1.)
