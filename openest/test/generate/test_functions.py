...
def test_Scale():
    ...
    for yearresult in application.push(DailyWeatherSlice(2000 * 1000 + np.arange(365), np.random.rand(365), manyregion=False)):
...
def test_Instabase():
    ...
    app1 = calc.test()
    app2 = calc.test()
    for yearresult in app1.push(DailyWeatherSlice(1800 * 1000 + np.arange(365), np.array([-10] * 300 + [10] * 65), manyregion=False)):
        np.testing.assert_equal(True, False) # Should get nothing here
    for yearresult in app2.push(DailyWeatherSlice(1800 * 1000 + np.arange(365), np.array([10] * 365), manyregion=False)):
        np.testing.assert_equal(True, False) # Should get nothing here
    for yearresult in app1.push(DailyWeatherSlice(2000 * 1000 + np.arange(365), np.array([-10] * 65 + [10] * 300), manyregion=False)):
        if yearresult[0] == 1:
            np.testing.assert_equal(yearresult[1], 65. / 300.)
        if yearresult[0] == 2:
            np.testing.assert_equal(yearresult[1], 1.)
    for yearresult in app2.push(DailyWeatherSlice(2000 * 1000 + np.arange(365), np.array([10] * 365), manyregion=False)):
        if yearresult[0] == 1:
            np.testing.assert_equal(yearresult[1], 1.)
        if yearresult[0] == 2:
            np.testing.assert_equal(yearresult[1], 1.)
