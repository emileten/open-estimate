...
def test_YearlyDayBins():
    ...
    for (year, result) in application.push(DailyWeatherSlice(1800 + np.arange(365), np.array([-10] * 300 + [10] * 65), manyregion=False)):
        np.testing.assert_equal(result, 65.)
    for (year, result) in application.push(DailyWeatherSlice(2000 + np.arange(365), np.array([-10] * 65 + [10] * 300), manyregion=False)):
        np.testing.assert_equal(result, 300.)

def test_Constant():
    application = Constant(33, 'widgets').test()
    for (year, result) in application.push(DailyWeatherSlice(1800 + np.arange(365), np.random.rand(365), manyregion=False)):
        np.testing.assert_equal(result, 33)
    for (year, result) in application.push(DailyWeatherSlice(2000 + np.arange(365), np.random.rand(365), manyregion=False)):
        np.testing.assert_equal(result, 33)
